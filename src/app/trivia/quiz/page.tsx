...
function loadQuestions(category: CategoryId, level: LevelId): TriviaQuestion[] {
  if (category === "capitals") {
    return generateCapitalsQuestions(level, TOTAL_QUESTIONS);
  }
  if (BANK_CATEGORIES.has(category)) {
...
function QuizInner() {
  ...
  const restart = useCallback(() => {
    ...
    setQuestions(loadQuestions(category, level));
    setCurrentIndex(0);
    setScore(0);
    ...
    return (
      <main className="mx-auto w-full max-w-4xl px-5 pb-20 pt-6 sm:px-8">
        ...
        <ResultScreen
          ...
          seconds={TIME_LIMIT_SECONDS - timeLeft}
          onRestart={restart}
          questions={questions}
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-extrabold tracking-tight text-ink sm:text-2xl">
            מבחן <span className="text-ink-soft">·</span>{" "}
...
function ProgressStrip({
  ...
  return (
    <div className="mt-6 flex items-center gap-4 rounded-2xl border border-line bg-white/85 p-4 shadow-[0_8px_24px_-16px_rgba(15,21,53,0.2)] backdrop-blur">
      ...
      <div
        className={`flex flex-shrink-0 items-center gap-1.5 text-sm font-bold tabular-nums ${
          urgent ? "text-rose-600" : "text-ink-soft"
        }`}
        dir="ltr"
      >
        <span>⏱️</span>
...
function ResultScreen({
  ...
  const newStars = pct >= 90 ? 40 : pct >= 70 ? 20 : pct >= 50 ? 10 : 0;
  ...
  return (
    <section className="mt-10 flex flex-col items-center text-center">
      ...
      <p className="mt-2 text-lg font-semibold text-ink-soft sm:text-xl">
        הצלחת ב-{score} מתוך {total} שאלות
      </p>

      <div className="mt-6 flex items-center gap-3 text-5xl sm:text-6xl">
        {[0, 1, 2].map((i) => (
          <span
            key={i}
            className={
      ...
      <p className="mt-4 text-sm font-semibold text-ink-soft" dir="ltr">
        ⏱️ {timeLabel}
      </p>
