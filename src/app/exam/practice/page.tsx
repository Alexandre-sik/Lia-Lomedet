...
import type { Question } from "@/lib/math-generator";
import { QuestionVisual } from "@/components/QuestionVisual";
import { recordPracticeComplete, useProgress } from "@/lib/progress";

const TOPIC_QUESTIONS = 12;
const MIXED_QUESTIONS = 15;

type Mode = ExamTopic | "mixed";

function isMode(v: string | null): v is Mode {
  return v === "mixed" || isExamTopic(v);
}

function modeLabel(mode: Mode): string {
  return mode === "mixed" ? "מבחן מעורב" : EXAM_TOPIC_LABELS[mode];
}

function buildQuestions(mode: Mode): Question[] {
  if (mode === "mixed") return generateMixedExam(MIXED_QUESTIONS);
  return generateExamQuestions(mode, TOPIC_QUESTIONS);
}
...
function PracticeInner() {
  ...
  const mode: Mode = isMode(topicParam) ? topicParam : "place-value";
  ...
  useEffect(() => {
    if (isComplete && !recordedRef.current) {
      ...
      recordPracticeComplete(`exam:${mode}`, "exam", pct, stars, startedAtRef.current);
    }
  }, [isComplete, score, questions.length, mode]);

  const restart = useCallback(() => {
    recordedRef.current = false;
    startedAtRef.current = Date.now();
    const next = buildQuestions(mode);
    setQuestions(next);
    setCurrentIndex(0);
    setScore(0);
    setUserAnswers(Array(next.length).fill(null));
    setSelectedAnswer(null);
    setShowFeedback(false);
    setIsComplete(false);
    setSeconds(0);
  ...
  if (isComplete) {
    return (
      <main className="mx-auto w-full max-w-4xl px-5 pb-20 pt-6 sm:px-8">
        <TopBar mode={mode} totalStars={progress.totalStars} />
        <ResultScreen
          score={score}
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-extrabold tracking-tight text-ink sm:text-2xl">
            הכנה למבחן <span className="text-ink-soft">·</span>{" "}
            <span className="text-primary">{modeLabel(mode)}</span>
          </h1>
        </div>
      </div>
...
function AnswerButton({
  ...
  return (
    <motion.button
      ...
      className={`relative flex min-h-[68px] items-center justify-center gap-3 rounded-2xl border-2 px-5 py-4 text-xl font-extrabold transition-all duration-200 sm:min-h-[76px] sm:text-[24px] ${stateClasses} ${
