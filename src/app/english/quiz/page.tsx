...
function QuizInner() {
  ...
  useEffect(() => {
    if (isComplete && !recordedRef.current) {
      ...
      recordQuizComplete(`en:${topic}`, level, pct, stars, startedAtRef.current);
    }
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-extrabold tracking-tight text-ink sm:text-2xl">
            English Quiz <span className="text-ink-soft">·</span>{" "}
            <span className="text-coral">{EN_TOPIC_LABELS[topic]}</span>
          </h1>
          <p className="text-sm font-semibold text-ink-soft">
            רמה: {EN_LEVEL_LABELS[level]} · 10 שאלות · 5 דקות
          </p>
        </div>
      </div>
        ...
        <div className="relative flex flex-col items-center">
          ...
          {question.prompt && (
            ...
            >
              ...
              {question.audio && (
                <SpeakButton text={question.audio} size="md" tone="white" />
              )}
            </div>
          )}
...
function QuizResult({
  ...
  const subhead = outOfTime
    ? `הספקת ${answered} מתוך ${total} שאלות.`
    : pct >= 70
    ? "Great English skills! Amazing progress."
    : "It's okay — every mistake is a learning opportunity.";
  ...
  return (
    ...
    >
        ...
        {[0, 1, 2].map((i) => {
          ...
          return (
            <motion.span
              ...
              className={filled ? "drop-shadow-[0_6px_14px_rgba(245,158,11,0.5)]" : ""}
              aria-hidden
            >
              {filled ? "⭐" : "☆"}
            </motion.span>
      ...
      >
        ...
        <Link
          href={`/english/practice?topic=${topic}&level=${level}`}
  ...
  return (
    <section className="mt-10 w-full max-w-3xl text-right">
      <h3 className="mb-4 text-xl font-extrabold text-ink sm:text-2xl">
        סקירה ({mistakes.length} טעויות
        {unanswered.length > 0 ? ` · ${unanswered.length} לא נענו` : ""})
      </h3>
