...
import { getEnLesson, type EnLesson, type EnSlide } from "@/lib/english-lessons";
import {
  EN_LEVEL_LABELS,
  isEnLevel,
  type EnLevel,
...
function LessonInner() {
  ...
  const id = params?.id ?? "";
  const lesson = getEnLesson(id);
  const levelParam = search.get("level");
  const level: EnLevel = isEnLevel(levelParam) ? levelParam : "easy";
...
function LessonView({ lesson, level }: { lesson: EnLesson; level: EnLevel }) {
  ...
  const practiceHref = `/english/practice?topic=${lesson.id}&level=${level}`;

  useEffect(() => {
    if (isLast && !recordedRef.current) {
      recordedRef.current = true;
      recordLessonComplete(`en:${lesson.id}`, `en:${lesson.id}`);
    }
  }, [isLast, lesson.id]);
  ...
  return (
    <main className="mx-auto w-full max-w-4xl px-5 pb-20 pt-6 sm:px-8">
      ...
      <LessonHeader
        ...
        levelLabel={EN_LEVEL_LABELS[level]}
      />
    ...
    <header className="flex flex-wrap items-center justify-between gap-3 py-2">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-extrabold tracking-tight text-ink sm:text-2xl">
          English Lesson <span className="text-ink-soft">·</span>{" "}
...
function LessonHeader({
  ...
  return (
    <section
      ...
      style={{
        background:
          "linear-gradient(135deg, #fb7185 0%, #f97316 55%, #fbbf24 100%)",
        boxShadow: "0 24px 60px -20px rgba(244, 63, 94, 0.55)",
      }}
    >
      <div
        ...
        return (
          <button
            ...
            className={`h-2.5 rounded-full transition-all duration-300 ${
              active
                ? "w-10 bg-coral shadow-[0_2px_10px_rgba(244,63,94,0.6)]"
                : done
                ? "w-2.5 bg-coral/60"
                : "w-2.5 bg-line"
            }`}
...
function SlideView({ slide, isLast }: { slide: EnSlide; isLast: boolean }) {
  return (
    <article className="rounded-[28px] border border-line bg-white p-7 shadow-[0_16px_40px_-20px_rgba(15,21,53,0.18)] sm:p-10">
      <div className="flex flex-col items-center text-center">
        ...
        <h3 className="mt-4 text-2xl font-extrabold tracking-tight text-ink sm:text-3xl" dir="auto">
          {slide.title}
        </h3>
        <p className="mt-3 max-w-2xl text-base font-medium leading-relaxed text-ink-soft sm:text-lg" dir="auto">
          {slide.body}
        </p>

        {slide.examples && slide.examples.length > 0 && (
          <div className="mt-6 w-full max-w-lg rounded-2xl border border-line bg-bg-2 p-5 text-right">
            ...
            <ul className="mt-3 flex flex-col gap-2">
              {slide.examples.map((ex, i) => (
                ...
                >
                  ...
                  <span className="flex-1 text-sm font-medium text-ink-soft" dir="rtl">
                    {ex.he}
                  </span>
        ...
        {isLast && (
          <div className="mt-6 text-sm font-semibold text-ink-soft">
            לחצי על 'Start practicing' למטה להמשיך ↓
          </div>
        )}
      </div>
    </article>
...
function NavRow({
  ...
  const accentBg =
    "linear-gradient(135deg, #fb7185 0%, #f97316 55%, #fbbf24 100%)";
  const accentShadow = "0 18px 40px -14px rgba(244, 63, 94, 0.55)";
  return (
